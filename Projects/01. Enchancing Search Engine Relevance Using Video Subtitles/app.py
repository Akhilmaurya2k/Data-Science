...
def record_audio(duration=40, samplerate=44100):
...
if toggle:
    audio, sr = record_audio()  # Record audio
    st.success("Recording complete!")

    # Convert NumPy array to a WAV format in-memory
    buffer = io.BytesIO()
    ...
    # Normalize the audio (Whisper expects float32 in range [-1, 1])
    samples /= np.iinfo(audio_segment.array_type).max

    # Convert to a PyTorch tensor (since Whisper expects torch.Tensor or NumPy array)
    audio_tensor = torch.from_numpy(samples)

    # Load the Whisper model and transcribe the in-memory audio
    model = whisper.load_model("base")
    result = model.transcribe(audio_tensor)

    # Save the transcribed text to a 'query' variable
    query = result['text']
    ...
    # st.subheader("Loading Symbol Example")
