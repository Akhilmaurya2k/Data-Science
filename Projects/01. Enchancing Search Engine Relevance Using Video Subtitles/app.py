...
def record_audio(duration=40, samplerate=44100, channels=1):
    ...
    with wave.open(wav_buffer, 'wb') as wf:
        ...
        wf.setframerate(samplerate)  # Sample rate
        wf.writeframes(audio_data.tobytes())  # Write the audio data to the buffer

    wav_buffer.seek(0)  # Move to the beginning of the buffer
    return wav_buffer
